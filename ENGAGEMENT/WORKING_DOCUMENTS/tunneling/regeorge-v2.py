...
class session(Thread):
    ...
    def setupRemoteSession(self, target, port):
        ...
        # response = conn.post(url=self.connectString + "?cmd=connect&target=%s&port=%d" % (target, port), headers=headers, data={})
        ...
        if response.status == 200:
            ...
            if status == "OK":
                ...
                # cookie = parse_jsession(response.getheader("set-cookie"))
